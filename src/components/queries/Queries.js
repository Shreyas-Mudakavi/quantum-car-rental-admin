...
export default function Queries() {
  ...
  const [
    { loading, error, queries, queriesCount, filteredQueriesCount },
    dispatch,
  ] = useReducer(queriesReducer, {
    loading: true,
    error: "",
  });
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          `/api/admin/getAllQueries?keyword=${query}&resultPerPage=${resultPerPage}&currentPage=${curPage}`,
          {
            headers: { Authorization: token },
          }
        );
                  ...
                  {loading ? (
                    ...
                    queries.map((query, i) => (
                      <tr key={query?._id} className="odd">
                        ...
                        <td>{query?.email}</td>
                        <td>{query?.message}</td>
                        <td>{getDateTime(query?.createdAt)}</td>
                        <td>
                          <Button
                            onClick={() => {
